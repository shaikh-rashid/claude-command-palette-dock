...
- Consolidated all authored Store-listing source under docs/store-listing/ (the listing CSVs and the screenshots with their HTML mockup sources), and made the two upload bundles — docs/store-submission/ (StoreBroker PDPs) and docs/store-import/ (Partner Center import) — generated-and-gitignored, regenerated from that source by the scripts rather than committed. This removes the triplicated screenshots and duplicated CSVs.
...
- Native Partner Center listing import: scripts/fill-store-listing-csv.ps1 fills a Partner Center "Export listing" CSV from the repo's listing content (default + per-language columns, screenshot paths and captions) for upload via Import listings → Upload folder.
- StoreBroker submission support: scripts/build-store-submission.ps1 generates a per-language ProductDescription PDP bundle (docs/store-submission/) from docs/store-listing/ for API-driven listing submission.
- A Screenshots section in README.md showing the four UI screenshots.
