...
/// month labels at the left where a new month starts, a separated WK column of week
/// totals, and a LESS→MORE color legend along the bottom — so the one graphic reads
/// day, week, and month usage. Rounded cells step through a teal→cyan intensity ramp
/// on a dark navy panel. Pixel work happens on a plain BGRA buffer and the OS PNG
/// encoder does the rest, so no drawing library is pulled in. The panel bakes in its
/// own dark surface so it reads the same on light and dark themes.
/// </summary>
internal static class TrendChartRenderer
{
    ...
    private const int CellCornerRadius = 6;
    ...
    private const int TotalsGap = 12; // extra space setting the WK column apart from the day grid
    ...
    private const int GridBottom = GridTop + WeekRows * CellSize + (WeekRows - 1) * CellGap;

    // The legend row: LESS, the five ramp swatches, MORE — right-aligned under the grid.
    private const int LegendCell = 14;
    private const int LegendCellGap = 4;
    private const int LegendGapAbove = 16;
    private const int LegendTextGap = 8;
    private const int LegendTop = GridBottom + LegendGapAbove;
    private const int LegendWidth = 2 * (4 * GlyphAdvance) + 2 * LegendTextGap + 5 * LegendCell + 4 * LegendCellGap;

    public const int DisplayWidth = TotalsLeft + CellSize + Padding;
    public const int DisplayHeight = LegendTop + LegendCell + Padding;
    ...
    // abbreviations (invariant culture), the WK header, and the legend can need.
    ...
    private static void DrawLegend(byte[] canvas, int w, int unit)
    {
        var pen = DisplayWidth - Padding - LegendWidth;
        var textY = (LegendTop + (LegendCell - GlyphSize) / 2) * unit;

        DrawText(canvas, w, "LESS", pen * unit, textY, unit);
        pen += 4 * GlyphAdvance + LegendTextGap;
        ...
        foreach (var color in CellRamp)
        {
            FillRoundedRect(
                ...
                color);
            pen += LegendCell + LegendCellGap;
        }

        pen += LegendTextGap - LegendCellGap;
        DrawText(canvas, w, "MORE", pen * unit, textY, unit);
    }
