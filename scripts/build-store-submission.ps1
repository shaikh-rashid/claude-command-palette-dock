...
#
# It reads the two authored CSVs (docs/store-listings.csv and
# docs/store-screenshot-captions.csv) and, for each language, writes a
# StoreBroker PDP (ProductDescription.xml). Screenshots go in Media/en-us and
# are shared across languages via StoreBroker's MediaFallbackLanguage.
#
# Re-run whenever the CSVs or screenshots change:  .\scripts\build-store-submission.ps1

$ErrorActionPreference = "Stop"

$repoRoot = Split-Path $PSScriptRoot -Parent
$docs = Join-Path $repoRoot "docs"
$listingsCsv = Join-Path $docs "store-listings.csv"
$captionsCsv = Join-Path $docs "store-screenshot-captions.csv"
$assets = Join-Path $docs "store-assets"
...
# Fresh generated subfolders (leave README.md and any hand-authored files alone).
